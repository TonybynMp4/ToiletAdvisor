		...
		.query(async ({ input }) => {
			...
			const query = db
				.select({
					...
					ratingCount: sql<number>`COUNT(DISTINCT ${rating.userId})`,
					mediaCount: sql<number>`COUNT(DISTINCT ${postMedia.id})`,
				})
				.from(post)
				.leftJoin(user, eq(post.userId, user.id))
				.leftJoin(rating, eq(post.id, rating.postId))
				.leftJoin(postMedia, eq(post.id, postMedia.postId))
			...
			const filtered = results.filter((r) => {
				if (input?.minRating && r.avgRating) {
					return Number(r.avgRating) >= input.minRating;
				}
				return true;
			});
